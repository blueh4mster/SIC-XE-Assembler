#include "../headers/OpCodeGenerator.h"
...
opCode generateOpCode(parsedLine pl, int loCtr, cSect* current, string& baseOp) {
	opCode toRet;
	...
	if (format == FORMAT_1) {
		...
		if (toRet.flags[0]) { //Extended? (20 bits)
			...
			if (findExpressionType(pl.operand1) == 6
				...
				return toRet;} //Numeric value after #
			if (disp >= -(1 << 11) && disp <= (1 << 11) - 1) { //Because PC is signed
				...
				int baseReg = -1;
				if (baseOp.compare("") !=0){
					baseReg = evaluateBaseOperand(baseOp, current);
				}
				if (baseReg == -1){
					throw new string("base register not set");
				}
				disp = address - baseReg;
				if (disp >= 0 && disp < 4096){    //base relative
					toRet.flags[2]=1;
					toRet.flags[1]=0;
					toRet.displacement = disp;
				} 
