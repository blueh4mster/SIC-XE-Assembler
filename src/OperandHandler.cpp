#include "../headers/OperandHandler.h"
...
bool isValidAlphaNumeric(string operand){
	if(operand.compare("*")==0)
		return true;
	for (unsigned int i = 0; i < operand.length(); i++)
		{if ((operand[i] < 'A' || operand[i] > 'Z')
...
bool isValidImmediateOp(string op) {
	...
	return isValidNum(rest) || isValidOpNm(rest) || isValidHexConstant(rest)
			|| isValidByteConstant(rest) || isValidAlphaNumeric(rest);
}
...
bool isValidIndirectOp(string op) {
	if (op.length() < 2 || op[0] != '@')
		return false;
	string rest = op.substr(1);
	return isValidNum(rest) || isValidOpNm(rest) || isValidAlphaNumeric(rest);
}
...
int evaluateBaseOperand(string operand, cSect* x) {
	int type = findExpressionType(operand);
	setCurCSECT(x);
	switch (type) {
	case 0: // #
	...
	case 6: // expression
		return evaluateExpression(operand, curCSECT, false);
	default:
		throw(string) "invalid operand for base";
	}
}
