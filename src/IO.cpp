...
string constructLine(int address, string label, string op, string operands[2]) {
	int length;
	...
	string ops = operands[0];
	if (operands[1] != ""){
		ops= ops + "," + operands[1];
	}
	adjustStringLength(&ops, length, spaceChar, false);
	line = lineNumberStr + addressStr + label + op + ops;
	return line;
}
...
void writeHeaderRecord(ofstream* file, string prog_name, int starting_add, int prog_length) { // 1
	...
	if (prog_name.compare("") != 6) adjustStringLength(&prog_name, 6, spaceChar, false);
	line += prog_name;
	line += "^";
