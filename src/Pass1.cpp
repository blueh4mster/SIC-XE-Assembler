...
void writeFomatted(parsedLine pl, cSect* current) {
	toWrite x;
	string operands[] = {pl.operand1,pl.operand2};
...
void writelts(cSect* current) {
	literalExistance = false;
	for (unsigned int i = 0; i < lts.size(); i++) {
		parsedLine p;
		p.lable = "*";
		p.opcode = trim(lts[i].operand1);
		p.operand1 = "";
		p.operand2 = "";
		p.err = "";
		writeFomatted(p, current);
		current->litTab.insert(
				pair<string, int>(trim(lts[i].operand1), current->locCtr));
		    //  Increment by the literal length
		  	incLocCtr(current, literalLength(lts[i].operand1, current));
...
void getlocCtrIncr(parsedLine pl, cSect* current, bool* started, bool* ended, string& baseOp) {
	int incr = validate(pl, baseOp);
	if (pl.operand1.length() > 0 && isLit(pl.operand1)) {
		bool add = true;
		for(unsigned int y = 0; y < lts.size() ; y++)
		{
			if(lts[y].operand1.compare(pl.operand1)==0){
				add = false;
				break;
			}
		}
...
cSect runPass1(vector<parsedLine> v, bool* started, bool* ended,
		bool* successfull, vector<string> _names, int idx, string& baseOp) {
	parsedLine pl;
	cSect current;
	current.name = _names[idx];
	current.locCtr = 0;
	lts.clear();
	literalExistance = false;
	for (unsigned int i = 0; i < v.size(); i++) {
		...
		try {
			getlocCtrIncr(pl, &current, started, ended, baseOp);
		} catch (string* e) {
			writeFomatted(pl, &current);
			*(successfull) = false;
			errorMessage += "\n";
